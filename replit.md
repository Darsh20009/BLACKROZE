...
- **System Menu Categories Seeded**: 13 system menu categories (7 drink: sys-hot/cold/specialty/basic/drinks/additional/desserts-drinks, 6 food: sys-food/sandwiches/bakery/croissant/cake/desserts-food) are auto-seeded into MongoDB on startup via `server/seed.ts` with `isSystem: true`. Admin Settings now shows these with a teal/orange "نظام" badge and disables the delete button for them; edit/reorder is allowed only for custom categories.
- **Inventory Live Tracking**: All inventory pages use `refetchInterval` (stock: 20s, alerts/purchases/transfers/movements: 30s, items/suppliers: 60s). Inventory hub has a live activity feed showing low-stock alerts and recent movements with timestamps.
- **POS Category Translation Fixed**: `visibleCategories` uses `t('menu.categories.*')` i18n keys with proper Lucide icons (Flame, Snowflake, Star, Cake, Utensils) instead of raw English strings.
- **Menu Subcategory Filtering Fixed**: `drinkCategoryIds` includes `additional_drinks`; `foodCategoryIds` includes `sandwiches`, `croissant`, `cake`; `matchesMode` logic works correctly for both departments.
